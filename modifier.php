  ...
  <body class="m-4">
      ...
      try {
        ...
        switch($_GET['choix']){
            default:
              ...
              while($r=$sQ->fetch()){
                  echo '<p>';
                  echo '<a href="modifier.php?choix=formulaire&id='.$r['id_morceau'].'">'.$r['titre'].'<span>('.$r['annee'].'-'.$r['genre'].'-'.$r['interprete'].')</span></a>';
                  echo '<p>';
              }
              break;
    ...
    <?php
      case 'formulaire':
        ...
        $r = $sQ->fetch();
    ?>
    <!-- call form for edit datas -->
    <form action="./save.php?action=edit" method="POST" class="p-5">
